...
public class Knob extends Button {
  ...
  // set size
    ...
    v = constrain_value(v,angle_min,angle_max);
    molette.angle(v);
  ...
  private void molette_update() {
    ...
    if(drag_direction != r.CIRCULAR) {
      ...
      new_angle = constrain_value(new_angle,angle_min,angle_max);
      ...
      //   angle = constrain_value(angle,angle_min,angle_max);
      // } else {
      //   angle = constrain_value(angle,angle_max,angle_min);
      // }
  ...
  private float constrain_value(float angle, float min, float max) {
    if(!open_knob) {
